            ...
            <div id="section-3">
              <h3>VEW PENDING REQUEST</h3>
                <?php
                    ...
                    $sql = "SELECT * FROM Result R, User U WHERE R.Status = 0;";
                    $result = mysqli_query($conn, $sql);

                    echo "<table id='req-table' border=1>";
                    echo "<tr><th> Student ID </th><th> Result ID </th><th> First Name </th><th>Last Name 
                            </th><th>Building No</th><th>Room No</th><th>Status</th><th>View</th></tr>";

                    if (mysqli_num_rows($result) > 0) {
                        while($row = mysqli_fetch_assoc($result)) {
                            echo "<tr><td>", $row["IdentificationID"], 
                            "</td><td>", $row["ResultID"], 
